...
export async function createPage(ctx) {
  ...
  async function onSubmit(event) {
    ...
    //take every single field out of FORM ==>
    ...
    if (
      ...
      return alert("Please fill all fields that are rquired!");
    }

    data.year = Number(data.year);
    data.price = Number(data.price);
    if (data.make.length < 4 || data.model.length < 4) {
      return alert("Make or Model must be at least 4 charecters long!");
    }
    if (data.year < 1950 || data.year > 2050) {
      return alert(
        "Invalid year! Year must be in range between 1950 and 2050!"
      );
    }
    ...
    if (data.price < 0) {
      return alert("Price can't be negative!");
    }
