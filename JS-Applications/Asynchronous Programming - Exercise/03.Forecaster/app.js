function attachEvents() {
  ...
  const currentForecastLabel = document.querySelector(
    "#forecast #current .label"
  );
  const upcomingForecastLabel = document.querySelector(
    "#forecast #upcoming .label"
  );
  ...
  async function fetchWeatherData() {
    ...
    try {
      ...
      const [searchedLocation] = data.filter(
        (e) => e.name.toLowerCase() === locationInput.value.toLowerCase()
      );
      ...
      await Promise.all([
        showTodayForecast(searchedLocation),
        showUpcomingForecast(searchedLocation),
      ]);
    } catch (e) {
    ...
    const todayForecast = create(
      ...
      create(
        ...
        create(
          ...
          `${low}${conditionChecker["Degrees"]()}/${high}${conditionChecker[
            "Degrees"
          ]()}`
        ),
  ...
  async function showUpcomingForecast(searchedLocation) {
    ...
    for (let day = 1; day <= data["forecast"].length; day++) {
      ...
      const currentDayForecast = create(
        "span",
        { className: "upcoming" },
