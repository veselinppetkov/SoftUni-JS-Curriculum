function solve() {
  ...
  function onClick() {
    ...
    let catalog = restaurants.reduce(
      (catalog, r) => reduceCatalog(catalog, r),
      {}
    );

    let sortedCatalog = Object.keys(catalog).sort(
      (key1, key2) => catalog[key2].avgPrice() - catalog[key1].avgPrice()
    );
    let bestRestaurant = catalog[sortedCatalog[0]];
    let sortedSalary = Object.keys(bestRestaurant.employees).sort(
      (key1, key2) =>
        bestRestaurant.employees[key2] - bestRestaurant.employees[key1]
    );

    let restaurantInfo = `Name: ${
      sortedCatalog[0]
    } Average Salary: ${bestRestaurant.avgPrice()} Best Salary: ${bestRestaurant.employees[
      sortedSalary[0]
    ].toFixed(2)}`;
    let workersInfo = "";
